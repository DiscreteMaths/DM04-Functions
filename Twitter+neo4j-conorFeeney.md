...
Every user in Twitter is given a unique user ID for their account. This is a numeric ID, so using a random number generator in order to remove bias, the user ID's of 189,000 people was successfully generated. Then using R and the established ``handshake" with the Twitter API, the user information was collected. R has a built in package called ``twitteR", which is specifically designed to work with the Twitter API, and using this the following information was obtained from each user:
